...
st.markdown(f"**Store:** {store} &nbsp;|&nbsp; **System:** Multi-Agent Food Waste Reduction Platform with SHAP Explainability")
st.divider()
...
if run_clicked:
    ...
    progress.progress(35, text="Agent 3/8 — Demand Forecast Agent training Random Forest + SHAP…")
    ...
    progress.progress(48, text="Agent 3/8 — Computing SHAP values for all products…")
    ...
    progress.progress(58, text="Agent 4/8 — Waste Prediction Agent estimating unsold units…")
    ...
    progress.progress(68, text="Agent 5/8 — Discount Agent computing tiered pricing…")
    discounts = discount_agent.analyze(waste_preds)
    log("Discount Agent", " | ".join([f"{d['Product']}:{d['Discount']}" for d in discounts]))

    progress.progress(78, text="Agent 6/8 — Sustainability Agent evaluating environmental impact…")
    ...
    progress.progress(95, text="Agent 8/8 — Explainability Agent generating report with SHAP insights…")
    ...
    log("Explainability Agent", "Business insight report with SHAP key drivers generated.")
    ...
    with tab5:
        ...
        # Build waterfall-style chart with Altair
        shap_rows = []
        cumulative = base_val
        ...
        # All products SHAP summary table
        ...
        # Feature importance bar from SHAP perspective (mean abs)
        fi_rows = []
